...
if ($method == 'POST') {
    ...
    if ($action == 'save_diagnosis') {
        ...
        $stmt = $pdo->prepare("UPDATE appointments SET status = 'diagnosed' WHERE id = ?");
    ...
    elseif ($action == 'save_prescription') {
        try {
            ...
            // 1. Tạo đơn thuốc
            $stmt = $pdo->prepare("INSERT INTO prescriptions (encounter_id, created_at) VALUES (?, NOW())");
            ...
            $stmt_stock = $pdo->prepare("UPDATE medicines SET stock_qty = stock_qty - ? WHERE id = ?");
            ...
            $stmt_find_app = $pdo->prepare("SELECT appointment_id FROM encounters WHERE id = ?");
            $stmt_find_app->execute([$encounter_id]);
            $app_id = $stmt_find_app->fetchColumn();

            $pdo->prepare("UPDATE appointments SET status = 'payment_pending' WHERE id = ?")->execute([$app_id]);
